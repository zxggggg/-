...
int main(int argc, char* argv[])
{
    ...
    int image_count=  22;/////参与定标的影像数       ///////////////////////每次运行前检查！
	//CvSize board_size = cvSize(5,8); /////标定板棋盘格内角点尺寸  
    CvSize board_size = cvSize(7,11); /////标定板棋盘格内角点尺寸    
	vector<Point2f> corners;  ///角点存储位置                
	vector<vector<Point2f> > corners_Seq; //////所有角点
	vector<Mat>  image_Seq;////所有影像
	int successImageNum = 0; ///成功读取的影像数               
	int count = 0;///提取的角点个数
    ...
    *************************************************************************/  
	...
	while(successImageNum < image_count){////读取所有影像
        ...
        cout<<"Frame #"<<successImageNum+1<<"..."<<endl;
        string imageFileName;
        std::stringstream StrStm;
        StrStm<<successImageNum+1;
        StrStm>>imageFileName;
        imageFileName += ".png";
        cv::Mat image = imread("./imagesBefore/fish"+imageFileName); /////读取影像，文件名为fish+对应影像的数字
        //if(!image.empty()){
        //    imshow("caliberation", image);
        //}
    ...
    //Size board_size = Size(7,11); /////标定板棋盘格内角点尺寸
	bool patternfound = findChessboardCorners(imageGray, board_size, corners, CALIB_CB_ADAPTIVE_THRESH );
        ...
        {   
            ...
            cv::imwrite("./imagechoose/" + imageFileName, imageTemp); // 输出提取角点后的图像
            ...
            //imwrite(imageFileName+".jpg",image);//输出原图像的jpg格式
            imageFileName += "_corner.jpg";
            //imwrite(imageFileName,imageTemp);
            cv::imwrite("./imagesCorner/" + imageFileName, imageTemp); // 输出提取角点后的图像
            cout<<"Frame corner#"<<successImageNum+1<<"...end"<<endl;
    ...
    *************************************************************************/   
    cout<<"开始定标………………"<<endl;  
    Size square_size = Size(49,49); //////定标板的格网边长为25×25mm    
    vector<vector<Point3f> >  object_Points;        /****  保存定标板上角点的三维坐标   ****/

    //cv::Mat image_points = cv::Mat(1, count, CV_32FC2, Scalar::all(0));  /*****   保存提取的所有角点   *****/
    ...
    *************************************************************************/
    ...
    for (int i = 0 ; i != image_count ; i++)
    {
        ...
        //imwrite(imageFileName,t);
         cv::imwrite("./imagesCalibrate/" + imageFileName, t); 
    }
    cout<<"保存结束"<<endl;
